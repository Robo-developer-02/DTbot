    ...
    try:
        ...
        for attempt in range(1, LLM_MAX_RETRIES + 2):  # +2 → 1 normal + N retries
            try:
                ...
                # PROMPT CACHING: Groq's cache is fully automatic (no request
                # flags needed) and reports how many prompt tokens were served
                # from cache via usage.prompt_tokens_details.cached_tokens.
                # That field may be absent (older SDK, or a model that doesn't
                # support caching yet) — getattr chain keeps this non-fatal.
                ...
                cache_hit_pct = (
                    ...
                    if usage.prompt_tokens else 0.0
                )

                logger.info(
                    "Tokens — prompt: %d | completion: %d | total: %d | "
                    "cached: %d (%.0f%% of prompt)",
...
def _transcribe_once(audio: np.ndarray, language: Optional[str] = None) -> Tuple[str, str]:
    ...
    passed as an explicit hint to the API.
    """
    ...
    try:
        ...
        logger.info(
            "STT [%s]: %.2fs for %.1fs of audio (%.2fx realtime)",
            STT_MODEL, stt_elapsed, audio_secs,
...
def transcribe(audio: np.ndarray) -> Tuple[str, str]:
    ...
    Uses whisper-large-v3. Only "en" and "hi" are ever returned — see
    FIX-P12 in _transcribe_once() for how everything else is folded in.
    """
...
def transcribe_fast(audio: np.ndarray) -> Tuple[str, str]:
    ...
    try:
        ...
        logger.info(
            "STT [%s]: %.2fs for %.1fs of audio (%.2fx realtime)",
            STT_MODEL_FAST, stt_elapsed, audio_secs,
            (stt_elapsed / audio_secs) if audio_secs else 0.0,
        )
    finally:
        if tmp_path and os.path.exists(tmp_path):
...
def print_banner(rag_en_ready: bool, rag_hi_ready: bool) -> None:
    ...
    banner = (
        f"\n{sep}\n"
        f"  DTBot 🤖  |  Regard Network Solutions, Delhi NCR\n"
        f"{sep}\n"
