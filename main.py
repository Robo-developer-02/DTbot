...
============================================================
  ...
  FIX-P10 USB speaker selection by name (mirroring MIC_NAME).
          Set SPEAKER_NAME in .env to select an output device by name
          (or substring). Leave empty to use the system default output.
          resolve_speaker_device() validates the name at startup and
          returns the device index needed by pygame.mixer.init().
============================================================
"""
...
# ── Speaker selection (FIX-P10) ───────────────────────────
# Set SPEAKER_NAME in your .env to select a speaker by name (or substring).
# Leave empty (or unset) to use the system default output device.
# Example: SPEAKER_NAME=USB Audio Device
# Run `python -c "import sounddevice as sd; print(sd.query_devices())"` to
# list all available device names on your system.
...
# ══════════════════════════════════════════════════════════
#  SPEAKER DEVICE RESOLUTION  (FIX-P10)
# ══════════════════════════════════════════════════════════

def resolve_speaker_device(name: str) -> Optional[int]:
    """
    Validate that *name* (or a substring of it) matches at least one output
    device, then return its integer index for use in pygame.mixer.init().

    Unlike sounddevice (which accepts name strings), pygame.mixer requires
    a device index — so we resolve the name to an index here at startup.

    Returns None when *name* is empty (falls through to system default) or
    raises RuntimeError when *name* is set but no matching output device is
    found.
    """
    if not name:
        logger.info("SPEAKER_NAME not set — using system default output device.")
        return None
    ...
    if not matches:
        ...
        raise RuntimeError(
            f"SPEAKER_NAME='{name}' did not match any output device.\n"
            f"Available output devices:\n" + "\n".join(available)
        )

    if len(matches) > 1:
        logger.warning(
            "SPEAKER_NAME='%s' matched %d devices — using the first: '%s'. "
            "Make your SPEAKER_NAME more specific if this is wrong.",
            name, len(matches), matches[0][1]["name"],
        )
    else:
        logger.info("Speaker resolved: '%s'", matches[0][1]["name"])

    # Return the device index; pygame.mixer.init() will use it directly.
    return matches[0][0]


# Resolve both devices once at module load so startup errors are caught
# immediately before any audio pipeline is opened.
_MIC_DEVICE:     Optional[str] = resolve_mic_device(MIC_NAME)
_SPEAKER_DEVICE: Optional[int] = resolve_speaker_device(SPEAKER_NAME)
...
def print_banner(rag_en_ready: bool, rag_hi_ready: bool) -> None:
    ...
    spk_label  = f"'{SPEAKER_NAME}'" if SPEAKER_NAME else "system default"
    sep = "=" * 60
    banner = (
        f"\n{sep}\n"
...
def main() -> None:
    try:
        # ── pygame mixer init with optional USB speaker (FIX-P10) ────
        if _SPEAKER_DEVICE is not None:
            device_info = sd.query_devices(_SPEAKER_DEVICE)
            sample_rate = int(device_info["default_samplerate"])
            device_name = device_info["name"]
            logger.info(
                "pygame.mixer: using device '%s' (index %d, %d Hz).",
                device_name, _SPEAKER_DEVICE, sample_rate,
            )
            pygame.mixer.init(frequency=sample_rate, devicename=device_name)
        else:
            pygame.mixer.init()
