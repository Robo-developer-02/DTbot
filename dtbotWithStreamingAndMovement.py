...
def extract_duration(text: str) -> int:
    ...
    seconds", "3 sec", "10 seconds"). Defaults to 1 second if no
...
# ══════════════════════════════════════════════════════════
#  STREAMING LLM → TTS (3-stage pipeline)
# ══════════════════════════════════════════════════════════
#
#  get_ai_reply() above is left completely unchanged and still works
#  standalone. get_ai_reply_stream() below has the same external
#  contract (validates input, retries on empty/API errors, appends to
#  history, returns the full reply string or raises) but streams the
#  LLM's tokens instead of waiting for the full response, through
#  THREE overlapping stages connected by two queues:
#
#    text_queue                          audio_queue
#  [Producer] ──chunks of text──▶ [Synthesizer] ──ready audio──▶ [Player]
#  reads LLM tokens              turns text into a playable      plays each
#  off the network                mp3 file via edge-tts           file, in order
#
#  The key fix this gives over a plain 2-stage producer/consumer: TTS
#  *synthesis* (a network call to edge-tts) is its own stage, separate
#  from *playback*. That means synthesis of sentence 2 starts the
#  moment sentence 1's synthesis is done — while sentence 1 is still
#  being played out loud — instead of only starting after sentence 1
#  finishes playing. That gap (dead air between sentences) is exactly
#  what this pipeline removes.
#
#  Movement (its own daemon thread, untouched) is unaffected either way.

_TTS_SENTINEL = object()  # propagated stage-to-stage to signal "no more input"


def _synthesize_to_file(text: str, voice: str) -> Tuple[Optional[str], float]:
    """
    Synthesize *text* via edge-tts into a new temp MP3 file. Pure
    synthesis only — no playback — so it can run in the synthesizer
    thread while the player thread is still playing an earlier chunk.
    Returns (path, gen_seconds), or (None, 0.0) on failure.
    """
    ...
    try:
        ...
        if not os.path.exists(tmp_path) or os.path.getsize(tmp_path) == 0:
            raise RuntimeError("edge-tts produced no/empty output.")
        return tmp_path, gen_time
    except Exception as exc:
        logger.error("edge-tts synthesis error: %s", exc)
        _cleanup_tmp_file(tmp_path)
        return None, 0.0


def _tts_synthesizer(text_queue: "queue.Queue", audio_queue: "queue.Queue", lang: str, errors: list) -> None:
    """
    Middle stage: pulls text chunks off text_queue and turns each into
    a ready-to-play audio file (or an espeak-fallback marker if offline
    / edge-tts fails), pushing the result onto audio_queue in the same
    order — this overlap with the player stage is what removes the gap
    between spoken sentences. Blocks on text_queue.get() (no polling).
    """
    while True:
        chunk = text_queue.get()
        try:
            if chunk is _TTS_SENTINEL:
                break
            voice = pick_voice(chunk, lang)
            if is_internet_available():
                path, gen_time = _synthesize_to_file(chunk, voice)
                if path:
                    audio_queue.put(("file", path, chunk, gen_time))
                    continue
                logger.warning("edge-tts failed for a chunk — falling back to espeak for it.")
            # Offline or edge-tts failed: fall back to espeak. espeak
            # synthesizes+plays in one blocking call, so this one
            # chunk can't be pipelined — the player stage runs it
            # synchronously, same as the old non-streaming fallback.
            audio_queue.put(("espeak", None, chunk, 0.0))
        except Exception as exc:
            logger.error("TTS synthesis error: %s", exc)
            errors.append(exc)
        finally:
            text_queue.task_done()
    audio_queue.put(_TTS_SENTINEL)   # always unblocks the player, even on error above


def _tts_player(audio_queue: "queue.Queue", lang: str, errors: list) -> None:
    """
    Final stage: pulls ready audio off audio_queue, strictly in order,
    and plays it — guaranteeing sentences are heard in the right order
    and never overlap, even though synthesis for later sentences may
    already be running concurrently in the synthesizer thread. Mic is
    muted for the entire streamed reply and released on the way out,
    even on error.
    """
    global _mic_muted
    _mic_muted = True   # gate mic BEFORE the first sentence plays
    try:
        while True:
            ...
            try:
                if item is _TTS_SENTINEL:
                    break
                kind, path, text, gen_time = item
                logger.info("TTS [%s]: %s", pick_voice(text, lang), textwrap.shorten(text, width=80))
                if kind == "file":
                    _last_tts_timing["gen"] = gen_time
                    t0 = time.time()
                    try:
                        ...
                        _cleanup_tmp_file(path)
                    _last_tts_timing["playback"] = time.time() - t0
                else:  # "espeak" — synchronous synth+play fallback for this chunk
        ...
        _mic_muted = False   # ALWAYS release the gate once the reply is fully spoken


def _llm_stream_producer(stream, text_queue: "queue.Queue", result: dict) -> None:
    """
    First stage: iterates the streamed LLM response (each `next()` on
    it blocks on network I/O, not CPU busy-waiting), buffers tokens,
    and puts each completed speakable chunk onto text_queue as soon as
    _split_into_speakable_chunks() says it's ready. Always signals
    _TTS_SENTINEL when done (success or failure) so the synthesizer
    stage can exit cleanly — never leaves it blocked forever.
    """
    full_reply = ""
    buffer = ""
    try:
        for event in stream:
            ...
            ready_chunks, buffer = _split_into_speakable_chunks(buffer)
            for chunk in ready_chunks:
                text_queue.put(chunk)

        leftover = sanitize_text(buffer)
        if not is_blank(leftover):
            text_queue.put(leftover)
    except Exception as exc:
        result["error"] = exc
    finally:
        result["full_reply"] = full_reply
        text_queue.put(_TTS_SENTINEL)   # always unblocks the synthesizer, even on error


def get_ai_reply_stream(user_text: str, lang: str, context: str) -> str:
    """
    Same external contract as get_ai_reply(): sends *user_text* to the
    LLM, validates input, retries on empty replies / API errors,
    appends the turn to history, and returns the full non-empty reply
    string (or raises on a genuine, exhausted API error).

    UNLIKE get_ai_reply(), this streams the model's tokens through a
    3-stage producer → synthesizer → player pipeline running
    concurrently — see the module docstring above — so TTS synthesis
    for the next sentence overlaps with playback of the current one,
    and the bot starts speaking almost immediately with no dead-air
    gap between sentences.

    Note: Groq's streaming responses don't reliably include token
    usage in-band the way non-streaming ones do, so token accounting
    (log_token_usage) is skipped for streamed calls — this only
    affects the informational token-usage log line, nothing else.
    """
    clean_input = sanitize_text(user_text)
    if is_blank(clean_input):
        raise ValueError("get_ai_reply_stream received empty or whitespace-only input.")
    ...
    try:
        ...
        while True:
            try:
                stream = client.chat.completions.create(
                    ...
                    reasoning_effort="low",
                    stream=True,   # ← the only behavioral switch vs get_ai_reply()
                )
                ...
                producer_thread = threading.Thread(
                    target=_llm_stream_producer,
                    args=(stream, text_queue, producer_result),
                    daemon=True,
                )
                synthesizer_thread = threading.Thread(
                    target=_tts_synthesizer,
                    args=(text_queue, audio_queue, lang, tts_errors),
                    daemon=True,
                )
                player_thread = threading.Thread(
                    target=_tts_player,
                    args=(audio_queue, lang, tts_errors),
                    daemon=True,
                )
                ...
                # Wait for all three stages to finish this turn —
                # graceful, no polling: join() blocks without
                # busy-waiting. Each stage always forwards its
                # sentinel in a finally block, so a failure anywhere
                # upstream still lets every downstream stage exit
                ...
                if tts_errors:
                    logger.warning(
                        "%d TTS error(s) occurred during streaming reply "
                        "(continuing — reply text is unaffected).",
                        len(tts_errors),
                    )
                ...
                full_reply = producer_result.get("full_reply", "")
                reply = sanitize_text(full_reply)
                if not is_blank(reply):
                    lang_history.append({"role": "assistant", "content": reply})
                    _trim_history(lang)
                    return reply

                # Empty stream — same "empty reply" retry semantics as
                # get_ai_reply(): nothing was spoken, so just retry.
                empty_retries += 1
                logger.warning(
                    "LLM stream returned empty response (empty attempt %d/%d) — retrying immediately.",
                    empty_retries, LLM_EMPTY_MAX_RETRIES,
                )
                if empty_retries >= LLM_EMPTY_MAX_RETRIES:
                    empty_exhausted = True
                ...
                api_retries += 1
                logger.warning("LLM streaming API error (attempt %d): %s", api_retries, api_exc)
                last_exc = api_exc
                if api_retries > LLM_API_MAX_RETRIES:
                    break
                wait = 2 ** api_retries
        ...
        if empty_exhausted:
            logger.warning(
                "LLM stream empty after %d attempts — speaking fallback instead of raising.",
                empty_retries,
            )
            lang_history.pop()
            speak(FALLBACK_REPLY, lang)   # nothing was spoken above, so speak it now
            return FALLBACK_REPLY

        lang_history.pop()
        raise last_exc or RuntimeError("LLM streaming failed after all retry attempts.")
...
# Sentence-ending punctuation used to decide when a chunk of streamed
# LLM text is "complete enough" to speak — English/most languages use
# ".", "!", "?"; Hindi/Sanskrit-derived scripts use "।" (danda) too.
_SENTENCE_END_CHARS = ".!?।"

# Soft cap (Req. 7): if the model produces a very long run with no
# sentence-ending punctuation (e.g. a long clause with commas, or a
# list), don't buffer indefinitely — speak what we have once it grows
# past this many characters, breaking at the nearest earlier space so
# words aren't cut mid-word.
MAX_CHUNK_CHARS = 200


def _split_into_speakable_chunks(buffer: str) -> Tuple[List[str], str]:
    """
    Given a running text buffer of streamed LLM output, split off any
    chunks that are ready to be spoken: complete sentences first, and —
    if the buffer grows past MAX_CHUNK_CHARS without hitting one — a
    length-based fallback split at the last word boundary so speech
    still starts within a bounded amount of text.

    Returns (ready_chunks, remainder) — ready_chunks is a list of
    complete strings ready for TTS (may be empty), remainder is the
    leftover partial text still waiting for more tokens.
    """
    chunks: List[str] = []
    start = 0
    i = 0
    while i < len(buffer):
        ...
        if ch in _SENTENCE_END_CHARS:
            ...
            start = i + 1
        elif (i + 1 - start) >= MAX_CHUNK_CHARS and ch == " ":
            # Long, unpunctuated run — break here instead of waiting
            # indefinitely for a sentence end that may never come soon.
            candidate = buffer[start:i].strip()
            if candidate:
                chunks.append(candidate)
            start = i + 1
        i += 1
...
def _unmute_mic() -> None:
    """Release the mic-mute gate. Used to guarantee the mic is never
    left muted, even if an exception happens mid-stream in the
    producer/consumer streaming TTS path (see get_ai_reply_stream())."""
    global _mic_muted
    _mic_muted = False


def speak(text: str, lang: str = "en", mute_after: bool = True) -> None:
    """
    Synthesize and play *text*.

    mute_after: when True (default — the normal single-utterance case),
    the mic is un-muted again as soon as this call finishes speaking.
    When False, the mic stays muted after this call returns — used by
    the streaming TTS consumer thread so the mic stays gated across an
    entire multi-sentence streamed reply instead of briefly un-muting
    between each spoken sentence. Callers passing mute_after=False are
    responsible for eventually calling _unmute_mic().
    """
    global _mic_muted
    ...
    try:
        ...
        if mute_after:
            _mic_muted = False     # ALWAYS release gate after playback (unless caller wants it held)
...
def _process_query(
    ...
    """
    Retrieve context for *user_text* and return (reply, already_spoken),
    or None on failure (error already announced inside this function).

    already_spoken is True only for the LLM-generation path, which now
    streams the model's tokens via a producer/consumer thread pair and
    speaks each completed sentence as soon as it's ready
    (get_ai_reply_stream()) — so by the time this function returns, the
    reply has already been spoken and the caller must NOT call speak()
    on it again. It's False for every other path (movement, stop, cache
    hit), which return instantly and still need the caller to speak the
    reply itself.
    ...
    *timings*, if given, is filled in-place with 'rag' and 'llm' elapsed
    ...
    if is_stop_command(clean):
        stop_movement()
        return stopped_reply(lang), False
    ...
    if is_move_command(clean):
        ...
        if duration > MAX_MOVE_DURATION:
            return duration_limit_reply(direction, lang), False
        move(direction, duration)
        return movement_reply(direction, duration, lang), False
    ...
    t0 = time.time()
    try:
        # Streams tokens via a producer (network reads) / consumer (TTS)
        # thread pair and speaks each completed sentence immediately —
        # the bot starts talking as soon as the first sentence is ready
        # instead of waiting for the full reply.
        reply = get_ai_reply_stream(clean, lang, context)
    except Exception as exc:
    ...
    try:
        ...
        while True:
            ...
            if state == State.LISTENING:
                ...
                timings: dict = {}
                result = _process_query(user_text, lang, rag_en, rag_hi, timings)
                ...
                reply, already_spoken = result

                state = State.SPEAKING

                # ── SPEAKING (inline, scoped to this reply) ───────────
                logger.info(state_label(state))
                # already_spoken=True means the streaming producer/
                # consumer path (get_ai_reply_stream) already spoke this
                # reply sentence by sentence as it was generated —
                # speaking it again here would say it a second time.
